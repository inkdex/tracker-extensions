...
export function getSessionToken(): string | undefined {
    const sessionToken = Application.getSecureState(STATE_MU_SESSION) as
        | string
        | null;
...
export function setSessionToken(sessionToken: string): void {
    if (typeof sessionToken !== "string") {
        console.log(
            `${logPrefix} tried to store invalid mu_sessiontoken: ${String(sessionToken)}`,
        );
...
export function getSessionInfo(): Session | undefined {
    try {
        ...
        return {
            ...
            loginTime: !isNaN(loginTime.getTime())
...
export function assertMustBeAuthenticated() {
    if (getSessionInfo() == null) {
        throw new Error(
            "You are not authenticated, please log in through the MangaUpdates settings",
        );
    }
}
