...
>(
    ...
    const path = Object.entries(baseRequest.params || {})
        ...
        .reduce((partialPath, [token, value]) => {
            if (!partialPath.includes(token)) {
                console.log(
                    `${logPrefix} endpoint '${endpoint}' does not contain ${token}!`,
                );
    ...
    if (failOnError && !ok) {
        console.log(
            `${logPrefix} failed ${meta}: ${loggableResponse(responseBody)}`,
        );
        throw new Error("Request failed!");
    }

    console.log(
        `${logPrefix} complete ${meta}: ${loggableResponse(responseBody)}`,
    );
    return responseBody;
}
