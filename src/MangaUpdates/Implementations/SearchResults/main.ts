    ...
    implements SearchResultsProviding
{
    async getSearchFilters(): Promise<SearchFilter[]> {
        ...
        return [
            ...
            },
            {
                ...
                placeholder:
                    'List the categories you want to include, separated by a ","',
                value: "",
            },
            {
                type: "multiselect",
    ...
    async getSearchResults(
        ...
        try {
            ...
            for (const filter of query.filters) {
                switch (filter.id as Filter) {
                    ...
                    case Filter.genre:
                        body.genre = [
                            ...(body.genre ?? []),
                            ...getMultiSelection(filter.value, "included").map(
                                (g) => g.replaceAll("_", " "),
                            ),
                        ];
                        body.exclude_genre = [
                            ...(body.exclude_genre ?? []),
                            ...getMultiSelection(filter.value, "excluded").map(
                                (g) => g.replaceAll("_", " "),
                            ),
                        ];
                        break;
                    ...
                    case Filter.licensed: {
                        const licensed = getBooleanSelection(
                            filter.value,
                            "licensed",
                        );
                        if (licensed != null) {
                    ...
                    case Filter.type:
                        body.filter_types = getMultiSelection(
                            filter.value,
                            "included",
                    ...
                    default:
                        console.log(
                            `${logPrefix} unknown filter: ${filter.id}`,
                        );
                        break;
                }
            }
            ...
            console.log(
                `${logPrefix} got results: ${JSON.stringify({ results, nextPage })}`,
            );
