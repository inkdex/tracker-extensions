...
export class MangaProgressImplementation implements MangaProgressProviding {
    ...
    async processChapterReadActionQueue(
        ...
        try {
            console.log(
                `${logPrefix} starts - received ${actions.length} action(s)`,
            );

            const mangaActions = actions.reduce<
                Record<string, TrackedMangaChapterReadAction[]>
            >((acc, action) => {
            ...
            if (updates.length > 0) {
                try {
                    ...
                    console.log(
                        `${logPrefix} applying list updates: ${JSON.stringify(body)}`,
                    );
            ...
            if (additions.length > 0) {
                try {
                    ...
                    console.log(
                        `${logPrefix} applying list additions: ${JSON.stringify(body)}`,
                    );
            ...
            return {
                ...
                failedItems: actions
                    .map((a) => a.id)
                    .filter((id) => !successfulItems.includes(id)),
            };
        }
    }
    ...
    async parseMangaActions(
        ...
        try {
            ...
            if (listInfo == null) {
                ...
                actionChapter > listChapter ||
                actionVolume > listVolume
            ) {
                type = "update";
            } else {
                type = "noop";
            ...
            console.log(
                `[parseAction] parse error: ${JSON.stringify(actions)}`,
            );
            console.log(e);
            return {
                ids: actions.map((a) => a.id),
