...
class DeletionForm extends Form {
            ...
            return [
                Section("deleted", [
                    LabelRow("deleted", {
                        title: success
                            ? "Deleted"
                            : this.result.status.toUpperCase(),
                        subtitle: success
    ...
    async onDelete() {
        ...
        try {
            ...
            console.log(
                `${logPrefix} complete: ${this.mangaId} - ${JSON.stringify(this.result)}`,
            );
        } catch (e) {
...
export class MangaProgressForm extends Form {
        ...
        return [
            ...
            Section({ id: "score", header: "Score" }, [
                StepperRow("score", {
                    title: "Score",
                    subtitle:
                        "Warning: Setting this to 0 will delete the rating from MangaUpdates.",
            ...
            Section(
                ...
                [
                    NavigationRow("delete", {
                        title: "Delete",
                        form: new DeletionForm(this.mangaId, (didDelete) =>
                            this.deletionFormClosed(didDelete),
                        ),
                    }),
                ],
            ),
    ...
    async loadData(overrideExistingState = false) {
        ...
        try {
            ...
            const [mangaInfo, mangaLists, progressInfo, ratingInfo] =
                await Promise.all([
            ...
            this.initialState = {
                ...
                listId:
                    progressInfo?.list_id != null
                        ? String(progressInfo.list_id)
                        : undefined,
            ...
            console.log(
                `${logPrefix} complete: ${this.mangaId} - ${JSON.stringify(this.initialState)}`,
            );
        } catch (e) {
            console.log(`${logPrefix} failed: ${this.mangaId}`);
            console.log(e);
    ...
    async saveChanges(): Promise<void> {
        ...
        try {
            ...
            if (progressChanged) {
                if (next.listId == null) {
                    ...
                    actions.push({
                        ...
                        action: () =>
                            makeRequest("/v1/lists/series", "POST", list),
                    });
                } else {
                    actions.push({
            ...
            for (const [i, { message, action }] of actions.entries()) {
                console.log(
                    `${logPrefix} action [${i + 1}/${actions.length}]: ${message}`,
                );
                if (actions.length > 1) {
                    this.loading = true;
